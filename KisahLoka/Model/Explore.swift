...
struct ResponseDataExplore : Codable {
    ...
    init(from decoder: Decoder) throws {
        ...
        data = try values.decodeIfPresent(DataExplore.self, forKey: .data)
        error = try values.decodeIfPresent(String.self, forKey: .error)
    }

}

struct DataExplore : Codable {
    let meta : Meta?
    let stories : [ExploreStories]?
    ...
    init(from decoder: Decoder) throws {
        ...
        stories = try values.decodeIfPresent([ExploreStories].self, forKey: .stories)
    }

}

struct ExploreStories : Codable, Hashable{
    let story_id : Int?
    let type_id : Int?
